...
export const useSimulacionStore = defineStore('simulacion', () => {
  ...
  const cargarParaEdicion = (datos) => {
    ...
    input.value.precioVivienda = Number(datos.precioVenta ?? datos.PrecioVenta ?? input.value.precioVivienda);
    input.value.plazoMeses = Number(datos.plazo ?? datos.Plazo ?? input.value.plazoMeses);
    ...
    input.value.tasaInteres = Number(datos.tasaInteres ?? datos.TasaInteres ?? 0);
    input.value.tipoTasa = datos.tipoTasa ?? datos.TipoTasa ?? 'Efectiva';
    input.value.frecuencia = datos.frecuencia ?? datos.Frecuencia ?? 'Mensual';
    ...
    input.value.periodosGracia = Number(datos.periodosGracia ?? datos.PeriodosGracia ?? 0);
    input.value.costesNotariales = Number(datos.costesNotariales ?? datos.CostesNotariales ?? 0);
    input.value.costesRegistrales = Number(datos.costesRegistrales ?? datos.CostesRegistrales ?? 0);
    input.value.tasacion = Number(datos.tasacion ?? datos.Tasacion ?? 0);
    input.value.portes = Number(datos.portes ?? datos.Portes ?? 0);
    input.value.gastosAdministracion = Number(datos.gastosAdministracion ?? datos.GastosAdministracion ?? 0);
    input.value.seguroDesgravamenMensual = Number(datos.seguroDesgravamenMensual ?? datos.SeguroDesgravamenMensual ?? 0);
    input.value.seguroRiesgoMensual = Number(datos.seguroRiesgoMensual ?? datos.SeguroRiesgoMensual ?? 0);
    ...
    input.value.aplicaBonoBuenPagador = Boolean(datos.aplicaBonoBuenPagador ?? datos.AplicaBonoBuenPagador ?? false);
    input.value.aplicaBonoVerde = Boolean(datos.aplicaBonoVerde ?? datos.AplicaBonoVerde ?? false);
    input.value.aplicaTechoPropio = Boolean(datos.aplicaTechoPropio ?? datos.AplicaTechoPropio ?? false);
